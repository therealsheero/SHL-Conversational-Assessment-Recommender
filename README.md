...
Once the server is running, visit **`http://localhost:8000`** in your browser to launch the interactive Web UI.
...
$$\text{Score}(d, q) = \sum_{t \in q} \left[ \min(\text{TF}_{q,t}, 2) \cdot (1 + \ln(\text{TF}_{d,t})) \cdot \text{IDF}_t \cdot W_{\text{name}} \right] + S_{\text{name\_match}} + S_{\text{phrase}} + S_{\text{skill}} + S_{\text{intent}} + S_{\text{duration}}$$
...
The application is completely self-contained and ready for deployment on platforms like **Render**, **Railway**, **Fly.io**, or **AWS/GCP/Azure**.
